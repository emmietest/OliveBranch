...
Highlight the current visible category in the sticky nav as user scrolls using IntersectionObserver.
</todo>

<todo id="more-menu-items">
Populate remaining category items
Pizza, Burgers, and Sushi sections each have only one item — add more to match a real menu.
</todo>

<todo id="cart-functionality">
Implement cart drawer/modal
The CartButton exists but has no functionality — wire up add-to-cart and a slide-out cart panel.
</todo>

<todo id="mobile-nav">
Add mobile hamburger nav
The header hamburger icon placeholder has no menu — add a slide-in mobile navigation drawer.
</todo>
